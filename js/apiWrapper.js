(function() {
    ...
    class ApiWrapper {
        ...
        getOptionsUrl() {
            if (!this.chr.runtime || !this.chr.runtime.id) {
        ...
        createBookmarks(bms) {
            ...
            const promise = new Promise((resolve, reject) => {
                ...
                let k = bms.length;
                bms.forEach(bm => {
                    self.chr.bookmarks.create({ parentId:bm.parentId, index:bm.index, title:bm.title, url:bm.url }, node => {
                        nodes.push(node);
                        k--;
                        if (k == 0)
        ...
        onMessage(listener) {
            ...
            const eh = new EventHandler(this.chr.runtime.onMessage, (request, sender, sendResponse) => {
                ...
                if (typeof(sendResponse) == 'function') {
                  if (result?.then) {
                    result.then(result=>sendResponse({result}));
                  } else {
                    sendResponse({result});
                  }
                }
